...
class TasksUpdateStatusHandler : public Poco::Net::HTTPRequestHandler {
public:
    void handleRequest(Poco::Net::HTTPServerRequest& request,
                       Poco::Net::HTTPServerResponse& response) override {
        // todo убрана для дебага
        // try {
        ...
        logger.information("Add_task_handler_logger: 200 POST /api/v1/task body whit is is %s", bodyStr);

        mai::service::TaskService service = mai::service::TaskService();

        Task task;
        try {
            task = mai::util::TaskParser::parseUpdateTask(bodyStr);
            logger.information("parced id %s", task._id);
            logger.information("parced status %s", task.status);
        } catch(...) {
        ...
        logger.information("was here");

        service.updateStatus(task._id, task.status);

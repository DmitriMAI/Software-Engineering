...
> Может тупить первый запуск, поэтому если не запущен сервис, то перезапустить еще раз docker compose -d --build

> файлы sql для задания в папке sql_for_bd

> Чтобы упросить процесс проверки дз - выключил JWT в методах, они доступны без авторизации.  
  
> Создаются только 2 таблицы из трех т.к третья будет для mongoDB Но для оптимизаций использовал и третью

> Предварительное создание бд, заполнение, в database/Database.h и при перезапуске 
docker compose up -d --build приходит в исходное состояние
